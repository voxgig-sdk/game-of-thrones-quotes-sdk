...
func (sdk *GameOfThronesQuotesSDK) OptionsMap() map[string]any {
...
func (sdk *GameOfThronesQuotesSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *GameOfThronesQuotesSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *GameOfThronesQuotesSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GameOfThronesQuotesSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *GameOfThronesQuotesSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *GameOfThronesQuotesSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("GameOfThronesQuotesSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *GameOfThronesQuotesSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GameOfThronesQuotesSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("GameOfThronesQuotesSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *GameOfThronesQuotesSDK {
	if sdkopts == nil {
	...
	sdk := NewGameOfThronesQuotesSDK(sdkopts)
	sdk.Mode = "test"
