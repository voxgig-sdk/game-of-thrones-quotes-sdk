# GameOfThronesQuotes SDK test runner
...
class GameOfThronesQuotesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GameOfThronesQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GameOfThronesQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GameOfThronesQuotesTestRunner.getenv("GAME_OF_THRONES_QUOTES_TEST_LIVE")
        override = GameOfThronesQuotesTestRunner.getenv("GAME_OF_THRONES_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GameOfThronesQuotesTestRunner.getenv(key)
        ...
        explain = GameOfThronesQuotesTestRunner.getenv("GAME_OF_THRONES_QUOTES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GameOfThronesQuotesTestRunner._test_control is not None:
            return GameOfThronesQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GameOfThronesQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GameOfThronesQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GameOfThronesQuotesTestRunner.load_test_control()
...
def load_env_local():
    GameOfThronesQuotesTestRunner.load_env_local()


def env_override(m):
    return GameOfThronesQuotesTestRunner.env_override(m)


def entity_data(v):
    return GameOfThronesQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GameOfThronesQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GameOfThronesQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GameOfThronesQuotesTestRunner.load_test_control()


def live_delay_ms():
    return GameOfThronesQuotesTestRunner.live_delay_ms()
