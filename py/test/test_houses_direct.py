...
class TestHousesDirect:

    def test_should_direct_load_houses(self):
        setup = _houses_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-houses", "live" if setup["live"] else "unit")
        if _skip:
...
def _houses_direct_setup(mockres):
