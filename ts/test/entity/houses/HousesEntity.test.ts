...
describe('HousesEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'houses.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GAME_OF_THRONES_QUOTES_TEST_HOUSES_ENTID JSON to run live')
      return
    }
    ...
    let houses_ref01_data = Object.values(setup.data.existing.houses)[0] as any

    // LOAD: skipped — no entity id field and load requires path params.
    // Entity-var is declared here so later flow steps still compile.
    const houses_ref01_ent = client.Houses()
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/houses/HousesTestData.json')
  ...
  const idmapEnvVal = process.env['GAME_OF_THRONES_QUOTES_TEST_HOUSES_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'GAME_OF_THRONES_QUOTES_TEST_HOUSES_ENTID': idmap,
    'GAME_OF_THRONES_QUOTES_TEST_LIVE': 'FALSE',
    'GAME_OF_THRONES_QUOTES_TEST_EXPLAIN': 'FALSE',
    'GAME_OF_THRONES_QUOTES_APIKEY': 'NONE',
  })

  idmap = env['GAME_OF_THRONES_QUOTES_TEST_HOUSES_ENTID']
