...
describe('HousesDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new GameOfThronesQuotesSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-load-houses', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-houses', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GAMEOFTHRONESQUOTES_TEST_HOUSES_ENTID']
