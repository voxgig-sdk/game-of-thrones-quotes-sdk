...
describe('HousEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GAME_OF_THRONES_QUOTES_TEST_HOUS_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GAME_OF_THRONES_QUOTES_TEST_HOUS_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'GAME_OF_THRONES_QUOTES_TEST_HOUS_ENTID': idmap,
    'GAME_OF_THRONES_QUOTES_TEST_LIVE': 'FALSE',
    'GAME_OF_THRONES_QUOTES_TEST_EXPLAIN': 'FALSE',
    'GAME_OF_THRONES_QUOTES_APIKEY': 'NONE',
  })

  idmap = env['GAME_OF_THRONES_QUOTES_TEST_HOUS_ENTID']
