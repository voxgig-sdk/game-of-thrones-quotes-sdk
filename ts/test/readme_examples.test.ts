...
//     (\`new GameOfThronesQuotesSDK(...)\` / \`GameOfThronesQuotesSDK.test(...)\` become
//     \`GameOfThronesQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'GameOfThronesQuotesSDK'
...
const TEST_SEED = {"entity":{"author":{"test01":{"id":"test01"}},"character":{"test01":{"id":"test01"}},"house":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GameOfThronesQuotesSDK(...)\` and any
// \`GameOfThronesQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GameOfThronesQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GameOfThronesQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
