...
boru-driven command-line client **and** interactive REPL for the GameOfThronesQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-of-thrones-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./game-of-thrones-quotes-cli --help
...
./game-of-thrones-quotes-cli list character

# 5. Override the API base URL for a single call
GAME_OF_THRONES_QUOTES_BASE=https://api.example.com ./game-of-thrones-quotes-cli list author

# 6. No arguments -> interactive REPL
./game-of-thrones-quotes-cli
game-of-thrones-quotes> list author
game-of-thrones-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-of-thrones-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/game-of-thrones-quotes-cli list author
   ```
...
Run with no arguments to open a REPL (prompt `game-of-thrones-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./game-of-thrones-quotes-cli
game-of-thrones-quotes> list author
game-of-thrones-quotes> /help
game-of-thrones-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/game-of-thrones-quotes-cli`. |
...
(`./game-of-thrones-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
