// Command GameOfThronesQuotes-cli is a CLI + REPL for the GameOfThronesQuotes SDK,
...
const prompt = "game-of-thrones-quotes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "author character house random"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GAME_OF_THRONES_QUOTES_APIKEY (api key), GAME_OF_THRONES_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GAME_OF_THRONES_QUOTES_APIKEY carries the API key and
	// GAME_OF_THRONES_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GAME_OF_THRONES_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GAME_OF_THRONES_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGameOfThronesQuotesSDK(opts)
