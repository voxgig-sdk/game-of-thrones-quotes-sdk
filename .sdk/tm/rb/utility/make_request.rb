...
module GameOfThronesQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GameOfThronesQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = GameOfThronesQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GameOfThronesQuotesResponse.new(fetched)
    else
