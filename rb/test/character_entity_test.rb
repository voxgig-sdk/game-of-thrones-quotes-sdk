...
class CharacterEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GAME_OF_THRONES_QUOTES_TEST_CHARACTER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def character_basic_setup(extra)
  ...
  entid_env_raw = ENV["GAME_OF_THRONES_QUOTES_TEST_CHARACTER_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "GAME_OF_THRONES_QUOTES_TEST_CHARACTER_ENTID" => idmap,
    "GAME_OF_THRONES_QUOTES_TEST_LIVE" => "FALSE",
    "GAME_OF_THRONES_QUOTES_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["GAME_OF_THRONES_QUOTES_TEST_CHARACTER_ENTID"])
