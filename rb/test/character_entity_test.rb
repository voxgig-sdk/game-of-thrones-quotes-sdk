...
class CharacterEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GAMEOFTHRONESQUOTES_TEST_CHARACTER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def character_basic_setup(extra)
  ...
  entid_env_raw = ENV["GAMEOFTHRONESQUOTES_TEST_CHARACTER_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "GAMEOFTHRONESQUOTES_TEST_CHARACTER_ENTID" => idmap,
    "GAMEOFTHRONESQUOTES_TEST_LIVE" => "FALSE",
    "GAMEOFTHRONESQUOTES_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["GAMEOFTHRONESQUOTES_TEST_CHARACTER_ENTID"])
