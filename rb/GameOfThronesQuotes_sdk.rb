...
require_relative 'GameOfThronesQuotes_types'


class GameOfThronesQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GameOfThronesQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GameOfThronesQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GameOfThronesQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GameOfThronesQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GameOfThronesQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GameOfThronesQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GameOfThronesQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GameOfThronesQuotesError.new(
        "#{op}_allow",
        "GameOfThronesQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue GameOfThronesQuotesError => err
    ...
    if fetched.is_a?(Hash)
      status = GameOfThronesQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GameOfThronesQuotesError.new(
        "graphql_error", "GameOfThronesQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GameOfThronesQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
