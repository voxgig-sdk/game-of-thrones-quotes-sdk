# game-of-thrones-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the GameOfThronesQuotes SDK as
two agent tools — `game-of-thrones-quotes_list` and `game-of-thrones-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-of-thrones-quotes-mcp)
make build

# 2. Provide credentials via the environment
export GAME_OF_THRONES_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user game-of-thrones-quotes \
  -- /absolute/path/to/game-of-thrones-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./game-of-thrones-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// game-of-thrones-quotes_list: first page of records
{ "entity": "author" }
{ "entity": "author", "query": { } }

// game-of-thrones-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-of-thrones-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user game-of-thrones-quotes \
     -- "$PWD"/dist/*/game-of-thrones-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `game-of-thrones-quotes_list` and `game-of-thrones-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list author using game-of-thrones-quotes"*
   and it calls `game-of-thrones-quotes_list` with `{"entity":"author"}`.
...
```sh
export GAME_OF_THRONES_QUOTES_APIKEY=sk_live_xxx            # API key
export GAME_OF_THRONES_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `game-of-thrones-quotes_list` tool
...
### Call the `game-of-thrones-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `game-of-thrones-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `game-of-thrones-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GAME_OF_THRONES_QUOTES_APIKEY` | API key sent with every request. |
| `GAME_OF_THRONES_QUOTES_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

author | character | house | random

### Smoke test via HTTP (raw JSON-RPC)

```sh
./game-of-thrones-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"game-of-thrones-quotes_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
