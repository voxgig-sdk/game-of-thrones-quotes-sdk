...
// GameOfThronesQuotes SDK feature test
//
...
class FtCtrl extends GameOfThronesQuotesControl
{
...
class FtClient
{
    ...
    private GameOfThronesQuotesUtility $utility;

    public function __construct(GameOfThronesQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): GameOfThronesQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GameOfThronesQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GameOfThronesQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GameOfThronesQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GameOfThronesQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(GameOfThronesQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GameOfThronesQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GameOfThronesQuotesResponse($fetched);
        }
    ...
    private function fail(GameOfThronesQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GameOfThronesQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
