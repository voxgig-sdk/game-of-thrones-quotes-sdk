...
// GameOfThronesQuotes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class GameOfThronesQuotesTestFeature extends GameOfThronesQuotesBaseFeature
{
    ...
    public function init(GameOfThronesQuotesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GameOfThronesQuotesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GameOfThronesQuotesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GameOfThronesQuotesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GameOfThronesQuotesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GameOfThronesQuotesParam::call($ctx, $k);
