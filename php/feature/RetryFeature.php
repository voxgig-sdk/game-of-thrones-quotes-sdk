...
// GameOfThronesQuotes SDK retry feature
...
class GameOfThronesQuotesRetryFeature extends GameOfThronesQuotesBaseFeature
{
    ...
    public function init(GameOfThronesQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GameOfThronesQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GameOfThronesQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GameOfThronesQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
