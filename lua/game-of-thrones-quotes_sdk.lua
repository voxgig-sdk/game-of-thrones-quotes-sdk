...
require("game-of-thrones-quotes_types")
...
local GameOfThronesQuotesSDK = {}
GameOfThronesQuotesSDK.__index = GameOfThronesQuotesSDK
...
GameOfThronesQuotesSDK._make_feature = _make_feature


function GameOfThronesQuotesSDK.new(options)
  local self = setmetatable({}, GameOfThronesQuotesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GameOfThronesQuotesSDK:options_map()
...
function GameOfThronesQuotesSDK:get_utility()
  return Utility.copy(self._utility)
end


function GameOfThronesQuotesSDK:get_root_ctx()
  return self._rootctx
end


function GameOfThronesQuotesSDK:prepare(fetchargs)
...
function GameOfThronesQuotesSDK:direct(fetchargs)
...
function GameOfThronesQuotesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function GameOfThronesQuotesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "GameOfThronesQuotesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function GameOfThronesQuotesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GameOfThronesQuotesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GameOfThronesQuotesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GameOfThronesQuotesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GameOfThronesQuotesSDK.new(sdkopts)
  sdk.mode = "test"
